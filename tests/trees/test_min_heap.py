...
class MinHeap_TestCase_Create_With_No_Element(unittest.TestCase):
    ...
    def test_heap_status(self):
        self.assertTrue(self._heap.is_min_heap, 'heap must be a min heap.')
        self.assertTrue(assert_min_heap_property(self._heap._get_heap_as_list()), 'heap property must be satisfied.')
...
class MinHeap_TestCase_Create_With_Predictable_SmallSet_Of_Elements(unittest.TestCase):
    ...
    def test_heap_status(self):
        self.assertTrue(self._heap.is_min_heap, 'heap must be a min heap.')
        self.assertTrue(assert_min_heap_property(self._heap._get_heap_as_list()), 'heap property must be satisfied.')
        self.assertTrue([None, 1, 3, 2] == self._heap._get_heap_as_list(), 'heap contents are not same as expected.')
...
class MinHeap_TestCase_1_Pop_With_Predictable_SmallSet_Of_Elements(unittest.TestCase):
    ...
    def test_heap_status(self):
        ...
        #
        self.assertTrue(assert_min_heap_property(self._heap._get_heap_as_list()), 'heap property must be satisfied.')
        self.assertTrue([None, 2, 3] == self._heap._get_heap_as_list(), 'heap contents are not same as expected.')
...
class MinHeap_TestCase_Iterator_Element(unittest.TestCase):
    """
    Create only. No heap modification. No Elements.
    """    
    def setUp(self):
